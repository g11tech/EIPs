...
Edge cases:
* For verkle at genesis, first block is block `1` and the history created will only have genesis hash corresponding to slot `0`
* for verkle activated at block `1`, first block is again block `1` and the history created will be same as above
* for verkle activated at block `32`, history created will be block `0..31`'s hashes.
## Rationale
